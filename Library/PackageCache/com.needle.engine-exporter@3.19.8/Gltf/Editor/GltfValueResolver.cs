		...
		public bool TryGetValue(IExportContext ctx, object instance, MemberInfo member, ref object value)
		{
			...
			if (ctx is GltfExportContext context)
			{
				...
				if (value is Font font && font)
				{
					...
					var outputPath = FontsHelper.TryGenerateRuntimeFont(font, style, context.AssetsDirectory, false, instance as Object);
					if (outputPath == null)
					{
						if (font)
				...
				if (value is Color col)
				{
					...
					// These colors are in sRGB space and are expected to be in sRGB space at runtime,
					// so we keep them as-is
					if (instance is Graphic || // panels, images, etc
					    instance is ColorBlock // buttons and other UI components with colors
					)
					{
						needsConversionToLinear = false;
						return true; // no need to log, explicitly handled
					}
