		...
		private class TypescriptHookDrawer : VisualElement
		{
			...
			private async void OnClick()
			{
				if (this.matchingTypescript != null)
				{
					...
					{
						WorkspaceUtils.OpenWorkspace(packageJsonDirectory, false, defaultEditor);
					...
					else
					{
						if (exportInfo != null)
						{
							WorkspaceUtils.OpenWorkspace(exportInfo.DirectoryName, false, defaultEditor);
							await Task.Delay(1000);
						}
					}
			...
			private void OnAttachToPanel(AttachToPanelEvent evt)
			{
				...
				if (types != null)
				{
					...
					if (match != null)
					{
						...
						this.postfix.style.color = new StyleColor(Color.gray);
						this.postfix.style.paddingLeft = 5;
						if(match.FilePath.Contains("@needle-tools/engine") || match.FilePath.Contains("@needle-tools\\engine"))
							this.postfix.text = match.PackageName;
