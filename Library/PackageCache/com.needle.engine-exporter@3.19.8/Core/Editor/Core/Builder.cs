	...
	public static class Builder
	{
			...
			BuildInfo? info = null)
		{
			...
			if (!await LicenseCheck.HasValidLicense())
			{
				...
				{
					trialLicenseMessage =
						"<b>Needle Engine Pro Trial has ended</b>: Please purchase a license to continue using all features of Needle Engine. " +
				...
				if (!authorized.success)
				{
					if (LicenseCheck.LastLicenseResult != true)
					{
						var msg = authorized.message ??
						          $"Needle Engine is not authorized to build the project. Please purchase a license at {Constants.BuyLicenseUrl.AsLink()}. If you believe this is a mistake, please contact hi@needle.tools.";
						Debug.LogError(msg);
						return false;
					}
				}
			...
			if (didSucceed)
			{
				SceneView.lastActiveSceneView?.ShowNotification(new GUIContent("Needle Engine: Export finished"), 1);
			}
			else
			{
				SceneView.lastActiveSceneView?.ShowNotification(
					new GUIContent("Needle Engine: Scene export failed (see console)"), 4);
			}
		...
		private static bool TryGetProjectInfo(bool isAutoExport, out ProjectInfo projectPaths, out ExportInfo info)
		{
			...
			if (infos.Length > 1)
				throw new Exception("Found multiple export infos in scene. Only one is allowed: " +
			...
			BuildInfo info)
		{
			...
			if (PlayerSettings.colorSpace != ColorSpace.Linear)
			{
				Debug.LogError("<b>Wrong colorspace</b>: " + PlayerSettings.colorSpace +
				               ", please set to Linear, otherwise your exported project will look incorrect. Change under \"Edit/Project Settings/Player/Other Settings\"");
			}
			...
			if (runExport)
			{
				const string registerTypesRelativePath = "./register_types.ts";
