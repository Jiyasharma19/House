	...
	internal static class ProjectInfoReporter
	{
		...
		private static async void InternalCollectDebugFiles(string header, CollectionSettings settings)
		{
			...
			try
			{
				...
				if (!string.IsNullOrWhiteSpace(humanProjectInfo)) description = humanProjectInfo + "\n\n" + description;
				...
				if (settings.Upload)
				{
					...
					else
					{
						...
						if (EditorUtility.DisplayDialog("Upload Bug Report", msg, "Yes, upload report (recommended)", "No, I will send the files manually"))
						{
							...
							{
								EditorUtility.DisplayCancelableProgressBar(header, "Uploading BugReport (" + sizeInMb + " MB)", progress);
								await Task.Delay(100);
								progress += (1 - progress) * .01f;
							}
							EditorUtility.DisplayCancelableProgressBar(header, "Finished Uploading BugReport (" + sizeInMb + " MB)", 1);
...
";
			...
			while (true)
			{
				...
				await Task.Delay(1000);
				var res = EditorUtility.DisplayDialog("Bug Report Description", reasonForDescriptionToBeRejected ?? "Please enter a description for the bug.\nReplace <TITLE> with a meaningful title for the bug.",
					"Continue...", "Open Description File");
				if (!res)
				{
					continue;
				}
				...
				if (description.Trim().StartsWith(defaultDescription.Trim()))
				{
					reasonForDescriptionToBeRejected =
						"Please describe the bug that you are observing to help us fix the issue...";
