...
namespace Needle.Engine
{
	public static class Actions
	{
		...
		public static async Task<bool> HasToktxInstalled(bool printError = true)
		{
			...
			if (File.Exists(ToktxUtils.GetToktxDefaultInstallationLocation() + "/toktx.exe"))
				return true;
#endif
			var cmd = "toktx --version";
			ToktxUtils.SetToktxCommandPathVariable(ref cmd);
			var cmdResult = ProcessHelper.RunCommandEnumerable(cmd);
			var isError = cmdResult.Any(l => l.Contains("'toktx' is not recognized") || l.Contains("command not found: toktx"));
			if (isError)
			{
				if (printError)
				{
		...
		public static async Task<bool> InstallCurrentProject(bool showWindow = false)
		{
			if (SceneExportUtils.IsValidExportScene(out var path, out _))
			{
				...
				var successfullyInstalledDependencies = await InstallDependenciesRecursive(path);
		...
		private static async Task<bool> InstallDependenciesRecursive(string directory, List<string> alreadyDiscoveredPaths = null)
		{				
				...
				foreach (var dep in dependencies)
				{
					if (PackageUtils.TryGetPath(directory, dep.Value, out var fullPath))
					{
						...
						if (!alreadyDiscoveredPaths.Contains(normalizedPath))
						{
							...
							if (File.Exists(npmDefPackageJsonPath)) versionHasChanged = await ProjectGenerator.UpdateVersions(npmDefPackageJsonPath);
							...
							if (needsInstalling || versionHasChanged)
							{
							...
							await InstallDependenciesRecursive(fullPath, alreadyDiscoveredPaths);
						}
					}
					else
					{
		...
		public static async Task<bool> InstallPackage(bool clean, bool showWindow = false, bool silent = false)
		{
			try
			{
				if (clean)
				...
				var success = await InstallCurrentProject(showWindow);
		...
		private static Task<bool> BeginInstall(string installCommand, string path, bool showWindow)
		{
			...
			SceneView.lastActiveSceneView?.ShowNotification(new GUIContent("Installing web-project " + pathDisplayName));
