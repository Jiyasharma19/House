...
namespace Needle.Engine.Samples
{
	public class SamplesWindow : EditorWindow, IHasCustomMenu
	{
		...
		private const string Unity2021Requirements = "Unity 2021.3.9+ and URP or BiRP";
		...
		internal static VisualElement CreateSampleView(List<SampleInfo> sampleInfos)
		{
			...
			if (reallyHaveSamples)
			{
				...
				// async void CheckForSampleUpdates()
				// {
				// 	// query available versions for the samples package from PackMan
				// 	var request = Client.Search(Constants.SamplesPackageName, true);
			...
			if (!HaveSamplesPackage && !CanInstallSamples)
			{
				buttonContainer.Add(new Button(() =>
				{
					...
					tooltip = $"The samples package requires either\n" +
					          $" · {Unity2020Requirements}\n" +
					          $" · {Unity2021Requirements}\n" +
					          $" · {NonLtsRequirements}\n" +
					          $"It's recommended to use the latest LTS version of either of these."
				});
			}
