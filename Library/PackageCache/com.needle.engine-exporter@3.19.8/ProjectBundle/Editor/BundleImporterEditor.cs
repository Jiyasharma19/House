		...
		private void InternalOnGUI()
		{
			...
			if (cache.TryGetValue(filePath, out var kvp))
			{
				...
				using (new GUILayout.HorizontalScope())
				{
					...
					var defaultTooltip =
						"The npm package can either be embedded (hidden but inside your Unity project, next to the npmdef), local (via some local path on your disc) or remote (being loaded from a package registry like npm, it is immutable in this case)";
					if (bundle.IsLocal)
					{
						value = "Local";
